...
for(let val of studentList){
  ...
  row.className = "table-row"
    ...
    clas.className = "sclass"
    let marks = document.createElement("td")
    marks.className = "marks"
    let passing = document.createElement("td")
    passing.className = "pass"
    let email = document.createElement("td")
    email.className = "email"
      id.innerText = `${val.id}`;
      sName.innerHTML = `<img src=${val.img_src} alt="photo" width="20" height="20">
       ${ val.first_name}  ${ val.last_name}`;
      gender.innerText = `${val.gender}`;
      clas.innerText = `${val.class}`;
      marks.innerText = `${val.marks}`;
      passing.innerText = `${ val.passing ? "Pass" : "Fail"}`
      email.innerText = `${val.email}`;
      //appned all data to row
      row.append(id)
      row.append(sName)
      row.append(gender)
      row.append(clas)
    ...
    table.append(row); // append row to the tbody of the table
}
...
document.getElementById("az").addEventListener("click",()=>{
  ...
  for(let val of azArr){
      ...
      document.querySelectorAll(".name")[i].innerHTML = `<img src=${val.img_src} alt="photo" width="20" height="20">
       ${ val.first_name}  ${ val.last_name}`;
       document.querySelectorAll(".gender")[i].innerText = `${val.gender}`;
       document.querySelectorAll(".sclass")[i].innerText = `${val.class}`;
...
marksArr.sort((a,b)=>{
 ...
 document.getElementById("gender").addEventListener("click",()=>{
  ...
  if(click > 1){
    ...
    table.remove();
    let mTable = document.querySelector("#male")
    mTable.setAttribute("border","1")
    mTable.innerHTML += `
    <caption>Male table</caption><thead>
    <tr>
      <th>Id</th>
      <th>Name</th>
      <th>Gender</th>
    ...
    let fTable = document.querySelector("#female")
    fTable.setAttribute("border","1")
    fTable.innerHTML += `
    <caption>Female List</caption>
    <thead>
    <tr>
      <th>Id</th>
      <th>Name</th>
    ...
    for(let val of studentList){
      ...
      row.className = "table-row"
        ...
        mTable.append(row); // append row to the tbody of the table
